...
export async function DELETE(
  ...
  try {
    ...
    console.error('時間入力削除エラー:', error);
    return NextResponse.json(
      { error: '時間入力の削除に失敗しました' },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    console.log('更新された時間入力:', timeEntry);
    return NextResponse.json(timeEntry);
  } catch (error) {
    console.error('時間入力更新エラー:', error);
    ...
    return NextResponse.json(
      { error: '時間入力の更新に失敗しました', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
}
